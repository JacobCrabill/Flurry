...
boundaries, and explicit Euler, RK44 and Jameson-RK time stepping.

Not yet: solution output (so a run reports residuals and nothing else), any
initial condition other than a uniform freestream, a CFL-derived time step
(`time.dt` has to be given), triangles, 3D elements, and the viscous terms —
which are written but unverified.
